...
int main(){
    ...
    while((fichierLu = readdir(rep)) != NULL)
    {
    	...
    	if(lastdot)
    	{
    		...
    		{
    			add = malloc(sizeof(cell));
    			add->val = strndup(fichierLu->d_name, strlen(fichierLu->d_name) - 6);
    ...
    printf("========== SO FILES =============\n");
    llist ptr = sofiles_head;
    llist ptr_robots;
    llist ptr_so = sofiles_head;
    int found = 0;
    char to_remove[300];
    while(ptr_so)
    {
    	ptr_robots = robotsfiles_head;
    	found = 0;
    	while(ptr_robots)
    	{
    		if(!strcmp(ptr_robots->val, ptr_so->val))
    		{
    			found = 1;
    			break;
    		}
    		ptr_robots=ptr_robots->next;
    	}
    	if(!found)
    	{
    		sprintf(to_remove, "%s.robot.so", ptr_so->val);
    		remove(to_remove);
